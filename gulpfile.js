...
async function themeCompile() {
  ...
  const sassComment = '// DO NOT EDIT THIS FILE!  It is an artifact compiled from gesso-theme-config.yml' + os.EOL;
  const sass = sassComment + renderSass(transformed.data);
  ...
  await Promise.all([
    ...
    writeFile(path.join(scssDir, '_gesso-theme-config.scss'), sass),
  ]);
}
...
function fileWatch() {
  ...
  watch(
    ['source/**/*.{twig,json,yaml,yml}', '!source/_patterns/00-config/gesso-theme-config.yml'],
    { usePolling: true, interval: 1500 },
    buildPatternlab,
  );
}
