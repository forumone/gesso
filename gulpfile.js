...
async function buildConfig() {
  ...
  const parsed = await readSource(
    path.join(__dirname, './source/_patterns/00-config/gesso-theme-config.yml'),
  );
  const plData = await readSource(
    path.join(__dirname, './source/_data/data_pl.yml'),
  );
  const dataComment = '# DO NOT EDIT THIS FILE.  This is a gitignored artifact.' + os.EOL +
    '# Global Pattern Lab variables should instead be edited in data_pl.yml.';

  const transformed = transform(parsed);
  const sassComment = '// DO NOT EDIT THIS FILE.  This is a gitignored artifact.' + os.EOL +
    '// Gesso style configuration variables should instead be edited in gesso-theme-config.yml.';

  await Promise.all([
    writeFile(
      path.join(ymlDir, 'data.yml'),
      dataComment + os.EOL + plData.source + yaml.stringify(transformed.data),
    ),
    writeFile(
      path.join(scssDir, '_gesso-theme-config.scss'),
...
function fileWatch() {
  watch(
    [
      'source/**/*.scss',
      'images/*.svg',
      '!source/_patterns/00-config/_gesso-theme-config.scss',
    ],
    { usePolling: true, interval: 1500 },
    series(lintStyles, buildStyles),
  );
  watch(
    ['source/_patterns/00-config/gesso-theme-config.yml'],
    { usePolling: true, interval: 1500 },
    series(
      buildConfig,
      parallel(
