...
const { isMap, isSeq, isPair } = require('yaml');
...
function getScalarVisitor(path) {
  ...
  return node => (node instanceof SassValue ? node : identityTransformer(node));
}

/**
 * @param {import('./types').ParsedSource<ParsedNode, true>} parsed
  ...
  function createDataObject(node, path) {
    ...
    if (isMap(node)) {
      ...
      for (const item of node.items) {
        if (!isPair(item)) {
          errors.push(
            map.errorForRange(`Can't handle non-pair '${item}'`, item.range)
          );
