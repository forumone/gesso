...
/**
 * Insert a library entry into a YAML file without reformatting it.
 * Preserves comments and spacing by editing the text directly.
 * @param {string} librariesFile
 * @param {string} componentName
 * @param {boolean} modularSass
 * @param {boolean} componentJs
 * @returns {Promise<void>}
 */
async function insertLibraryEntry(
  ...
  const contents = await readFile(librariesFile, { encoding: 'utf-8' });
  ...
  const sortedKeys = topLevel.slice().sort(compareKeys);
  const nextKeyIndex = sortedKeys.findIndex(
    key => compareKeys(componentName, key) < 0
  );
  const nextKey = nextKeyIndex === -1 ? null : sortedKeys[nextKeyIndex];
  let insertIndex = nextKey ? keyIndex.get(nextKey) : lines.length;

  if (insertIndex > 0 && lines[insertIndex - 1].trim() === '') {
    insertIndex -= 1;
  }

  const entryLines = [`${componentName}:`];
  if (modularSass) {
    entryLines.push('  css:', '    theme:', `      dist/css/${componentName}.css: {}`);
  }
  if (componentJs) {
    entryLines.push('  js:', `    dist/css/${componentName}.js: {}`);
  }
  entryLines.push('  dependencies:', '    - gesso/global');

  const insertLines = [];
  if (insertIndex > 0 && lines[insertIndex - 1].trim() !== '') {
    insertLines.push('');
  }
  insertLines.push(...entryLines);
  if (nextKey && (lines[insertIndex]?.trim() !== '')) {
    insertLines.push('');
  }

  lines.splice(insertIndex, 0, ...insertLines);
  await writeFile(librariesFile, lines.join(EOL), {
    encoding: 'utf-8',
    flag: 'w+',
  });
}
