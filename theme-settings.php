...
function gesso_form_system_theme_settings_alter(array &$form, FormStateInterface $form_state, ?string $form_id = NULL): void {
  ...
  /** @var \Drupal\Core\Extension\ThemeSettingsProvider $themeSettings */
  $themeSettings = \Drupal::service('Drupal\Core\Extension\ThemeSettingsProvider');

  $form['back_to_top'] = [
    '#type' => 'details',
    '#title' => t('Back to Top'),
    '#open' => $themeSettings->getSetting('include_back_to_top') ?? TRUE,
  ];
  $form['back_to_top']['include_back_to_top'] = [
    '#type' => 'checkbox',
    '#title' => t('Include back to top'),
    '#default_value' => $themeSettings->getSetting('include_back_to_top') ?? TRUE,
  ];
  ...
  $form['breadcrumb']['include_current_page_in_breadcrumb'] = [
    ...
    '#default_value' => $themeSettings->getSetting('include_current_page_in_breadcrumb') ?? TRUE,
  ];
  ...
  $form['external_links']['add_external_link_icons'] = [
    ...
    '#default_value' => $themeSettings->getSetting('add_external_link_icons') ?? FALSE,
  ];
  $form['external_links']['exit_disclaimer'] = [
    '#type' => 'textfield',
    '#title' => t('Exit Disclaimer'),
    '#description' => t('Disclaimer text used for exit links.'),
    '#default_value' => $themeSettings->getSetting('exit_disclaimer') ?? 'Exit this website',
  ];
  $form['external_links']['allowed_domains'] = [
    ...
    '#default_value' => $themeSettings->getSetting('allowed_domains') ?? "example-allowed-domain.com\nforumone.github.io",
  ];
  $form['external_links']['allowed_links'] = [
    ...
    '#default_value' => $themeSettings->getSetting('allowed_links') ?? "https://www.vimeo.com/example-allowed-link\nhttps://www.youtube.com/example-allowed-link",
  ];
}
