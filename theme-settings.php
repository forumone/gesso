...
function gesso_form_system_theme_settings_alter(array &$form, FormStateInterface $form_state, ?string $form_id = NULL): void {
  ...
  $form['back_to_top'] = [
    '#type' => 'details',
    '#title' => t('Back to Top'),
    '#open' => gesso_helper_get_theme_setting('include_back_to_top') ?? TRUE,
  ];
  $form['back_to_top']['include_back_to_top'] = [
    '#type' => 'checkbox',
    '#title' => t('Include back to top'),
    '#default_value' => gesso_helper_get_theme_setting('include_back_to_top') ?? TRUE,
  ];
  $form['back_to_top']['threshold'] = [
    ...
    '#default_value' => gesso_helper_get_theme_setting('threshold') ?? 200,
  ];
  $form['back_to_top']['smooth_scroll'] = [
    '#type' => 'checkbox',
    '#title' => t('Enable smooth scroll'),
    '#description' => t('Whether to animate the scroll back to the top'),
    '#default_value' => gesso_helper_get_theme_setting('smooth_scroll') ?? TRUE,
  ];
  ...
  $form['breadcrumb']['include_current_page_in_breadcrumb'] = [
    ...
    '#default_value' => gesso_helper_get_theme_setting('include_current_page_in_breadcrumb') ?? TRUE,
  ];
  ...
  $form['external_links']['add_external_link_icons'] = [
    ...
    '#default_value' => gesso_helper_get_theme_setting('add_external_link_icons') ?? FALSE,
  ];
  $form['external_links']['exit_disclaimer'] = [
    '#type' => 'textfield',
    '#title' => t('Exit Disclaimer'),
    '#description' => t('Disclaimer text used for exit links.'),
    '#default_value' => gesso_helper_get_theme_setting('exit_disclaimer') ?? 'Exit this website',
  ];
  $form['external_links']['allowed_domains'] = [
    ...
    '#default_value' => gesso_helper_get_theme_setting('allowed_domains') ?? "example-allowed-domain.com\nforumone.github.io",
  ];
  $form['external_links']['allowed_links'] = [
    ...
    '#default_value' => gesso_helper_get_theme_setting('allowed_links') ?? "https://www.vimeo.com/example-allowed-link\nhttps://www.youtube.com/example-allowed-link",
  ];
}
